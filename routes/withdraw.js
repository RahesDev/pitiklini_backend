...
var antiPhishingDB = require("../schema/antiphising.js");
var sms = require("../helper/sms");
var SettingDB = require("../schema/sitesettings.js");
const { processWithdrawal , processWithdrawal2 } = require('../controller/processwithdraw.js');
const { validateWithdrawal , validateWithdrawal2 } = require('../middleware/validateMiddleware.js')
...
const generateRandomOTP = () => {
  ...
  const createWithdrawal = async (userId, currencySymbol, otp, withdrawalAddress, withdrawalAmount) => {
    const withdrawal = new withdrawDB({
    user_id: userId,
    currency_symbol: currencySymbol,
    withdrawOTP: otp, 
    withdraw_address: withdrawalAddress,
    expireTime: moment().add(2, 'minutes').toDate(),
...
router.post('/fieldvalidate', common.tokenmiddleware, validateWithdrawal, async (req, res) => {
  ...
  if (!errors.isEmpty()) {
    ...
    return res.json({ status: false, errors: errors.array(), message: errors.array()[0].msg });
  }
  
  try {
  const userId = req.userId;
  const { currency_symbol, withdrawalAddress, withdrawalAmount } = req.body;
  ...
  await createWithdrawal(userId, currency_symbol, generatedOtp, withdrawalAddress, withdrawalAmount);  
  const dynamicSubject = `Your Withdrawal OTP for ${currency_symbol}`;  
  let resData = await mailtempDB.findOne({ key: "withdraw_otp" });
  var findDetails = await antiPhishingDB.findOne({ userid: userId });
  var message = `Antiphising Code - ${findDetails && findDetails.APcode ? findDetails.APcode : ""}`
  ...
  router.post('/fieldvalidateAdmin', common.tokenmiddleware, validateWithdrawal2, async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      console.log(errors.array()[0].msg)
      return res.json({ status: false, errors: errors.array(), message: errors.array()[0].msg });
    ...
    try {
      const userId = req.userId;
      const { currency_symbol, withdrawalAddress,networkType } = req.body;
      ...
      const decryptedEmail = common.decrypt(user.email);
      console.log(decryptedEmail,"decryptedEmail")
      const generatedOtp = generateRandomOTP();
      await createWithdrawal(userId, currency_symbol, generatedOtp, withdrawalAddress, userName,networkType);
      const dynamicSubject = `Your Withdrawal OTP for ${currency_symbol}`;
      let resData = await mailtempDB.findOne({ key: "withdraw_otp" });
      var findDetails = await antiPhishingDB.findOne({ userid: userId });
      var message = `Antiphising Code - ${findDetails && findDetails.APcode ? findDetails.APcode : ""}`
...
router.post('/process', common.tokenmiddleware,validateWithdrawal, processWithdrawal);
router.post('/processAdmin', common.tokenmiddleware, validateWithdrawal2, processWithdrawal2);
...
//   try {
//     const pageSize = Number(req.body.pageSize) || 5; // Default page size
//     const page = Number(req.body.currentPage) || 1;
//     // const filterKeyword = req.body.filterKeyword || "";
//     const skip = (page - 1) * pageSize;

//     // Build the match query with optional filterKeyword
//     let matchQuery = {
//       type: 0,
//       withdraw_type: 0,
//       status: { $in: [1, 2, 3, 4] },
...
//     // if (filterKeyword) {
//     //   matchQuery["user_name"] = { $regex: filterKeyword, $options: "i" };
//     // }

//     // Get the total count of documents matching the query
//     const totalCount = await withdrawDB.countDocuments(matchQuery);
...
//         {
//           $lookup: {
//             from: "users", // Adjust this if your user collection is named differently
//             localField: "user_id", // Replace this with the actual field name linking to the user
//             foreignField: "_id",
//             as: "user_detail",
//           },
//         },
...
//           preserveNullAndEmptyArrays: true, // This will keep documents even if currency_detail is empty
//           }, },
//         {
//           $project: {
//             currency_symbol: 1,
...
//             created_at: 1,
//             // user_name: 1,
//             user_name: "$user_detail.displayname", // Use displayname from userDB
//             email: 1,
//             currency_name: { $ifNull: ["$currency_detail.currencyName", "Unknown"] }, // Provide a default value if currency_detail is empty
//           },
//         },
//         { $sort: { _id: -1 } },
//         { $skip: skip },
//         { $limit: pageSize },
//       ])
//       .exec((err, withdrawdata) => {
//         console.log(withdrawdata,"---withdrawdata datas--");
//         if (!err && withdrawdata) {
...
//now original -->
// router.post("/get_all_user_deposit", common.tokenmiddleware, async(req, res) => {
//   try {
//     // Extract page and limit from the request body
...
//         const totalCount = await depositDB.countDocuments({ status: 1 });
...
//           var userdeposits = deposits.map((deposit) => ({
//             _id: deposit._id,
//             username: deposit.username,
//             txnid: deposit.txnid.substring(0, 15) + "...",
//             date: deposit.date,
...
//           // Return the paginated data and total records for frontend to calculate pages
//           return res.status(200).json({
//             status: true,
//             data: userdeposits,
...
//       });
//   } catch (e) {
//     console.log("get_user_deposit catch", e);
//     return res.status(500).json({
//       status: false,
//       data: [],
//       message: "Something went wrong",
...
router.post("/get_all_user_deposit", common.tokenmiddleware, async (req, res) => {
  try {
    ...
    if (filterKeyword) {
      matchQuery["user.displayname"] = { $regex: filterKeyword, $options: "i" }; // Case-insensitive search
    }

    // Calculate the total count of documents matching the filter
    const totalCount = await depositDB.aggregate([
...
router.get("/getCurrenciesDatas", async (req, res) => {
  try {
      const activeCurrencies = await currencyDB.find(
          { status: "Active" },
          { currencyType: 1,coinType:1, currencyName: 1, currencySymbol: 1 ,_id:1,Currency_image:1,
             maxSwap: 1, minSwap: 1, swapFee: 1 , swapStatus: 1
          }
        ).lean(); // Use lean() to get plain JavaScript objects instead of Mongoose documents for better performance
        
        if(activeCurrencies.length > 0){
          var currencies = [];
          for (var i = 0; i < activeCurrencies.length; i++){
            if (activeCurrencies[i].swapStatus == "1") {
            var obj = {
              currencyName: activeCurrencies[i].currencyName,
              currencySymbol: activeCurrencies[i].currencySymbol,
              currencyType: activeCurrencies[i].currencyType,
              coinType: activeCurrencies[i].coinType,
              image: activeCurrencies[i].Currency_image,
              currid: activeCurrencies[i]._id,
              maxSwap: activeCurrencies[i].maxSwap,
              minSwap: activeCurrencies[i].minSwap,
              swapFee: activeCurrencies[i].swapFee,
            };
            currencies.push(obj);
          }
          }
        ...
        // return res.status(200).json({ status: true, currency: coins,fiat:tokens });
