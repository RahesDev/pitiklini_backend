                      ...
                      if (updateFromBal) {
                        ...
                        if (updateTomBal) {
                          var obj = {
                            ...
                            receivedAmount: +req.body.toAmount,
          ...
          if (walletData) {
            ...
            if (checkBalance == true) {
              ...
              var notifyDatas = {
                ...
                message: "Internal transfer done successfully",
                link: "/internaltransferhistory",
                status: 0,
                viewed: 0,
              };
              ...
              if (transferSuccess) {
                await transferDB.create(obj);
                await notify.create(notifyDatas);

                // Fetch user's email from DB
                const user = await usersDB.findOne({ _id: userId });
                if (!user) {
                  return res.json({ status: false, Message: "Transfer failed, please try again later" });
                }
                const userEmail = common.decrypt(user.email);
                var findDetails = await antiPhishing.findOne({ userid: user._id });
                var message = (findDetails != null) ? `Antiphising Code - ${findDetails.APcode}` : "";
                console.log(findDetails, "findDetails----message", message);
                // Fetch the email template
                const emailTemplate = await mailtempDB.findOne({ key: "internalTransfer" });
                if (!emailTemplate) {
                  return res.json({ status: false, Message: "Transfer failed, please try again later" });
                }

                // Replace dynamic placeholders in the email template
