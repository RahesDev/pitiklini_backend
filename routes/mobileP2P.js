      ...
      if (p2pdet) {
        ...
        if (cancel) {
          ...
          if (userBalance != null) {
            ...
            const updateholdbalance = await common.updateHoldAmount(
              cancel.userId,
              cancel.fromCurrency,
              updateHold
            );
...
router.post("/getp2pchat", common.tokenmiddleware, async (req, res) => {
  try {
    ...
    return res.json({ status: true, Message: getChat.length ? getChat : [] });
  } catch (error) {
...
router.post("/dispute_order", common.tokenmiddleware, async (req, res) => {
  try {
    ...
    const from_user = await usersDB
      ...
      .exec();
    const opp_detail = await usersDB
      .findOne(
        {
          _id:
            p2pdet.userId.toString() === userId.toString()
              ? p2pdet.userId
              : p2pdet.map_userId,
        },
        { email: 1, displayname: 1 }
    ...
    const notification = await notifyDB.create({
      ...
      status: 0,
      type: "p2p",
      message: `${from_user.displayname} has raised dispute for the order ${p2pdet.askAmount} ${p2pdet.fromCurrency} ${p2pdet.type} order`,
      link: `/p2p/chat/${p2pdet.orderId}`,
    });
    ...
    if (savechat) {
      const getChat = await p2pChatDB
        .findOne({ orderId: savechat.orderId })
        .limit(1)
        .sort({ _id: -1 })
        .exec();
      common.sendResponseSocket(
