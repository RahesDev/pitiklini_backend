...
var BINANCEexchange = require("../exchanges/binance");
...
status: true,
Message: "",
todayProfit: totalUSDTBalance ==0 ? 0.00 :totalUSDTBalance,
yesterdayProfit: totalUSDTBalanceYesterday ==0 ?0.00 :totalUSDTBalanceYesterday,
});
  ...
  try{
    ...
    if(!getstakedata){
      ...
      if (+nextDay < +currentDate) {
        ...
        return res.json({status: false, Message:"Now you cannot make claim your amount, please try after one day "});
      }
    }
  ...
  try {
    ...
    if (currency_id != null && currency_id != undefined) {
      var balanceData = await userWalletDB.findOne(
        ...
        {
        wallets: { $elemMatch: { currencyId: mongoose.Types.ObjectId(currency_id) } },
        }
        );
        
        if (balanceData && balanceData.wallets && balanceData.wallets[0]) {
        var walletData = balanceData.wallets[0];
        console.log(walletData, "Retrieved wallet data >>>>>>>>>>>>>>>>>>>>>>>");
        var obj = {
        ...
        amount_rptc20: walletData.amount_rptc20
        };
        var currency_balance = obj.totalBalance;
        return res.json({
          status: true,
          balance: currency_balance.toFixed(4),
        });
      }
